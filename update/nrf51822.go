...
func (m *Nrf51822) ExtractPayload(sync chan StatusResponse, resp StatusResponse) (StatusResponse, error) {
...
func (m *Nrf51822) Update(sync chan StatusResponse, resp StatusResponse) (StatusResponse, error) {
	if err := initialise(); err != nil {
		return resp, err
	} else if client, err := bluetooth.Connect(m.LocalUUID, 10); err != nil {
		return resp, err
	} else if err := m.subscribe(client); err != nil {
		return resp, err
	} else {
		defer client.ClearSubscriptions()
		if err := m.checkFOTA(client); err != nil {
			return resp, err
		} else if m.Firmware.currentBlock == 0 {
			if err := m.initFOTA(client); err != nil {
				return resp, err
			}
		} else if err := m.transferFOTA(client); err != nil {
			return resp, err
		} else if err := m.validateFOTA(client); err != nil {
			return resp, err
		} else if err := m.finaliseFOTA(client); err != nil {
			return resp, err
		}
	}
